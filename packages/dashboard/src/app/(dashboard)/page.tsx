...
export default function DashboardPage() {
  ...
  const maxDaily = Math.max(...(dailySales.length > 0 ? dailySales.map((d) => d.value) : [1]));
  ...
  return (
    <div className="space-y-6">
              ...
              return ordersByStatus.map((s) => (
                <div key={s.label} className="flex items-center justify-between text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-slate-400 text-xs">({Math.round((s.count / total) * 100)}%)</span>
                  </div>
                </div>
              ));
            })()}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Categories */}
        <div className="bg-white rounded-xl border border-slate-200 p-5">
          <h2 className="font-semibold text-slate-900 mb-4">Productos más vendidos</h2>
              ...
              return (
                <div key={p.name}>
                  <div className="flex items-center justify-between text-sm mb-1.5">
                    ...
                    <span className="text-slate-500 font-semibold">{formatCurrency(p.total)}</span>
                  </div>
