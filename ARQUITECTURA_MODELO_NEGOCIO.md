...
| **Tienda online por farmacia** | Canal web sin proyecto aparte | Storefront Next.js sobre nuestra API (`/store/:storeId`, público) — reusa búsqueda Meili + pipeline de pedidos; pedidos web caen en el MISMO buzón `/orders` (tag `web`). Pago contra entrega. | ⚠️ código completo 2026-06-07; falta verificación prod + imágenes de producto |
| Post-MVP | Estándar de operación + valor agregado | Conector POS real (ADR-007/008), email de credenciales, landing, recordatorios de refill a crónicos, facturación SaaS, **tracking del pedido en mapa** (link al despachar) | 💡 |
